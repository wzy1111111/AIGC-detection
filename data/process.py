...
# from network.denoising_rgb import DenoiseNet
...
def custom_resize(img, opt):
    ...
    return TF.resize(img, opt.loadSize,interpolation=InterpolationMode.BILINEAR)
...
def processing_DCT(img,opt,processing_model):
    ...
    cropped_img = torch.from_numpy(dct2_wrapper(img, processing_model)).permute(2,0,1).to(dtype=torch.float)
    return cropped_img
...
def processing_DIRE(img,diffusion_model,diffusion,opt,imgname):
