...
class Encoder(nn.Module):
    ...
    def forward(self,feature_in: torch.Tensor,feature_bags:torch.tensor):
        ...
        # feature_in2=self.norm1(self.multiHeadAttention2(feature_in1,feature_in,feature_in))#B 257 1024  B 257 1024
...
class LowFeature(nn.Module):
    ...
    def forward(self,semantic_map:torch.Tensor, x: torch.Tensor):
        ...
        x_c_1=F.relu((torch.abs(x_s_1-x_1)*(torch.abs(x_s_1-x_1))*x_1+x_1))
        ...
        x_c_2=F.relu((torch.abs(x_s_2-x_2)*(torch.abs(x_s_2-x_2))*x_2+x_2))

        x_3=F.relu(self.bn3(self.conv3(x_c_2)))#B 28 28 1

        return x_3.squeeze(-1).view(B, -1)
...
class GCP(nn.Module):
    ...
    def forward(self, x:torch.Tensor,feature_in: torch.Tensor,feature_bags:torch.tensor):
        ...
        feature_out_1=self.decoder(feature_in1,feature_in2)
        difference_part=torch.abs((feature_out_1-feature_in)[:,1:,:])
        extract_feature=self.linear(self.lowFeature(difference_part,x))
       
        return feature_out_1,extract_feature
...
class SSD(nn.Module):
    ...
    def forward(self, x,feature_bags=None):
        ...
        prior_features=torch.load('../datasets/feature_bags_5000_final.pt').reshape(1,-1,feature_clip.shape[2]).to(x.device)
        fusion_feature=feature_clip
        restoration_feature,final_feature=self.gcp(x,fusion_feature,prior_features)
        ...
        return output,final_feature,restoration_feature,feature_clip
